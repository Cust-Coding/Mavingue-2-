...
import { listWaterCustomers, listWaterContracts, listWaterReadings, listWaterBills } from "@/features/water/api";
import type { WaterCustomer, WaterContract, WaterReading, WaterBill } from "@/features/water/types";
...
export default function AguaPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const [c, ct, l, f] = await Promise.all([
        listWaterCustomers(),
        listWaterContracts(),
        listWaterReadings(),
        listWaterBills(),
  ...
  return (
    <div>
      ...
      {!loading && tab === "clientes" && (
        <>
          {clientes.length === 0 ? <Empty text="Sem clientes de água." /> : (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #eee" }}>Ferragem ID</th>
                  </tr>
                </thead>
                <tbody>
                  {clientes.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{c.contacto}</td>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{c.ferragemId}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {contratos.map((c) => (
                    <tr key={c.id}>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{c.id}</td>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{c.customerId}</td>
            ...
            <div style={{ border: "1px solid #ddd", borderRadius: 10, background: "white", overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #eee" }}>ID</th>
                    <th style={{ textAlign: "left", padding: 10, borderBottom: "1px solid #eee" }}>Contrato ID</th>
                ...
                <tbody>
                  {leituras.map((l) => (
                    <tr key={l.id}>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{l.id}</td>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{l.contractId}</td>
            ...
            <div style={{ border: "1px solid #ddd", borderRadius: 10, background: "white", overflow: "hidden" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {faturas.map((f) => (
                    <tr key={f.id}>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{f.id}</td>
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>{f.contractId}</td>
                      ...
                      <td style={{ padding: 10, borderBottom: "1px solid #f3f3f3" }}>
                        <span style={{ 
                          background: f.status === "PAID" ? "#dcfce7" : "#fef3c7", 
