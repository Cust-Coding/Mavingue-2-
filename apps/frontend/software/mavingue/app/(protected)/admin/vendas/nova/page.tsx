...
export default function NovaVenda() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([productsApi.list(), customersApi.list(), categoriesApi.list()])
      .then(([productRows, customerRows, categoryRows]) => {
        setProducts(productRows);
        setCustomers(customerRows);
        setCategories(categoryRows);
      })
      .catch((reason: unknown) => setError(getErrorMessage(reason, "Nao foi possivel carregar produtos, clientes e categorias")))
            ...
            <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 grid gap-3">
                ...
                <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-2 text-3xl font-black text-orange-700">{formatMoney(total)}</div>
                </div>
                <div className="rounded-[24px] border border-emerald-200 bg-emerald-50 p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-700">Troco</div>
                  <div className="mt-2 text-3xl font-black text-emerald-700">{formatMoney(troco)}</div>
                </div>
              </div>
